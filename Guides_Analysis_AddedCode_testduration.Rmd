...
for (i in 1:numr)
{
  ...
  dfNU_GVSC$Segment[i] = dfr1$segmentId[i]
  nsteps = dfr1$numSteps[i]
  nstepval = nsteps - i
  ...
  if ((FC3 > 0)&(dfr1$stepIndex = nsteps)) #Logic to read max and min time
  {
    dfNU_GVSC$EndTime[i] = min(dfr1$browserTime);
  }
...
p2 <- ggplot(dfRepeats,aes(x = Num_Steps, Rate, y = percent_repeat_views_by_length))
   p2 + geom_point(stat = 'identity',color = "#FF4876")+
...
p3 <- ggplot(dfRepeats,aes(x = Num_Steps, Rate, y = percent_repeat_visitors_by_length))
   p3 + geom_line(stat = 'identity',color = "#FF4876")+
    labs(x="Guide length (steps)", y= "% Repeat views", na.rm = TRUE)+
    geom_smooth(method = "lm", se = FALSE, colour= "#0A817F")+
